...
- No profiles at all → tell the user to run `cfswitch wizard` **themselves, in their own interactive terminal** — never run it from an agent sandbox (it opens their browser and watches their clipboard). It auto-creates verified, pinned profiles the moment they copy the token. `cfswitch wizard --print-url` gives just the pre-filled URL. (Manual fallback: create tokens at https://dash.cloudflare.com/profile/api-tokens and `cfswitch add`.)
